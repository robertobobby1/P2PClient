    ...
    class Client {
       public:
        ...
        bool run(const char *hostname, int port) {
            ...
            if (server == NULL) {
                onError(clientSocket, false, "[Client] ERROR getting host name");
                return false;
            }
            ...
            if (connect(clientSocket, (struct sockaddr *)&socketAddress, sizeof(socketAddress)) < 0) {
                onError(clientSocket, false, "[Client] Couldn't connect to the host");
                return false;
            }
        ...
        void terminate() {
            onError(clientSocket, true, "[Client] Closing the client socket!");
        }

        void sendMessage(Buffer buff) {
            Net::sendMessage(clientSocket, buff, "[Client] Couldn't send message");
        }

        Buffer readMessage(Socket socket) {
            return Net::readMessage(clientSocket, "[Client] Couldn't read message");
        }
    };
    ...
    class Server {
       public:
        ...
        bool run(int port = 3000, int backlog = 10) {
            ...
            if (checkForErrors(bind(serverSocket, (sockaddr *)&serverAddress, sizeof(serverAddress)), -1, "[Server] Error on socket binding", true)) {
                return false;
            }

            if (checkForErrors(listen(serverSocket, backlog), -1, "[Server] Error while starting to listen on port", true)) {
                return false;
            }
        ...
        bool run(int port = 3000, int backlog = 10) {
            ...
            if (checkForErrors(serverSocket, INVALID_SOCKET, "[Server] Error on socket creation", false))
                return false;

            // If error on socket binding it may mean that the port is in use, we can search a new one!
            if (checkForErrors(bind(serverSocket, (SOCKADDR *)&service, sizeof(service)), SOCKET_ERROR, "[Server] Error on socket binding", true))
                return false;

            if (checkForErrors(listen(serverSocket, 1), SOCKET_ERROR, "[Server] Error while starting to listen on port", true))
                return false;

            unsigned long blocking_mode = 0;
            if (checkForErrors(ioctlsocket(serverSocket, FIONBIO, &blocking_mode), -1, "[Server] Error while setting the blocking mode", true))
                return false;
        ...
        void terminate() {
            onError(serverSocket, true, "[Server] Closing the server socket!");
        }
        ...
        Socket acceptNewConnection() {
            ...
            if (checkForErrors(AcceptSocket, SocketError, "[Server] Error while accepting new connections", true))
                return -1;
        ...
        void sendMessage(Buffer buff) {
            Net::sendMessage(serverSocket, buff, "[Server] Couldn't send message");
        }
